...
const TourTable = ({ isLoading, currentTours, filteredLength, onTourUpdated, onTourDeleted }) => {
    ...
    const handleDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            await axiosClient.delete(`/tours/${tourToDelete.tour_id}`);
            toast.success(`Đã xóa tour “${tourToDelete.title}” thành công!`);
            setTourToDelete(null);
            onTourDeleted?.(tourToDelete.tour_id);
        } catch (err) {
            toast.error('Đã xảy ra lỗi khi xóa tour: ' + (err.response?.data?.message || err.message));
        } finally {
            setIsDeleting(false);
        }
    };
                                ...
                                <tr key={tour.tour_id} className="border-t last:border-b hover:bg-gray-50">
                                    ...
                                    <td className="py-3 px-3 text-center">
                                        <div className="flex items-center gap-2 justify-center">
                                            ...
                                            />
                                            <Edit
                                                className="w-4 h-4 text-orange-500 cursor-pointer hover:scale-110 transition"
