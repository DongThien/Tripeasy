...
const ForgotPassword = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (!email) {
            toast.error('Vui lòng nhập email để nhận mã khôi phục');
            return;
        }
        try {
            ...
            if (success) {
                toast.success(message || 'Đã gửi mã khôi phục');
                return;
            }

            toast.error(message || 'Gửi mã khôi phục thất bại');
        } catch (error) {
            const message = error?.response?.data?.message || 'Gửi mã khôi phục thất bại';
            toast.error(message);
        }
    };
            ...
            <div className="grid min-h-screen lg:grid-cols-2">
                <section className="flex min-h-screen flex-col bg-[#FDFBF7] px-6 py-10 md:px-10 lg:px-14">
                    ...
                    <div className="flex flex-1 items-center">
                        <div className="w-full max-w-md mx-auto">
                            <h1 className="text-5xl font-bold tracking-tight text-[#111827]">Quên mật khẩu?</h1>
                            <p className="mt-3 text-lg text-gray-500">
                                Nhập email để nhận mã khôi phục tài khoản của bạn.
                            </p>
